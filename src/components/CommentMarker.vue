...
import { DIMENSIONS, TIMEOUTS } from "../constants";
...
const markerPosition = computed<Position>(() => {
  // Force recomputation on resize or scroll
  updateTrigger.value;

  const calculateFromElement = (): Position | null => {
    if (!props.comment.elementSelector) {
      return null;
    }
    ...
    // Use viewport coordinates for position: fixed
    return {
...
function handleMarkerEnter(): void {
  clearHideTimeout();
...
// Immediate update without debounce for initial mount
const handleImmediateUpdate = (): void => {
  updateTrigger.value++;
};
